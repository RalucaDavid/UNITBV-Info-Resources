...
EMovementType g_movementType = EMovementType::None;

void RenderFunction()
{
	static float increment = 0;
	increment++;
	float angleValue = increment / 1000;
	...
	case EMovementType::Horizontal:
		worldTransf = glm::translate(glm::mat4(1.0), glm::vec3(glm::sin(angleValue), 0.0f, 0.0f));
		break;
	case EMovementType::Vertical:
		worldTransf = glm::translate(glm::mat4(1.0), glm::vec3(0.0f, glm::sin(angleValue), 0.0f));
		break;
	case EMovementType::Circle:
		worldTransf = glm::translate(glm::mat4(1.0), glm::vec3(glm::sin(angleValue), glm::cos(angleValue),0.0f));
		break;
	case EMovementType::AroundZ:
		worldTransf = glm::rotate(glm::mat4(1.0), angleValue, glm::vec3(0.0f, 0.0f, 1.0f));
		break;
	case EMovementType::Scale:
		worldTransf = glm::scale(glm::mat4(1.0), glm::vec3(glm::abs(glm::sin(angleValue)), glm::abs(glm::sin(angleValue)), glm::abs(glm::sin(angleValue))));
		break;
	default:
		break;
	}
...
{
	if (tolower(Key) == 'n') {
		g_movementType = EMovementType::None;
	}
	if (tolower(Key) == 'h') {
		g_movementType = EMovementType::Horizontal;
	}
	if (tolower(Key) == 'v') {
		g_movementType = EMovementType::Vertical;
	}
	if (tolower(Key) == 'c') {
		g_movementType = EMovementType::Circle;
	}
	if (tolower(Key) == 'z') {
		g_movementType = EMovementType::AroundZ;
	}
	if (tolower(Key) == 's') {
		g_movementType = EMovementType::Scale;
	}
}
