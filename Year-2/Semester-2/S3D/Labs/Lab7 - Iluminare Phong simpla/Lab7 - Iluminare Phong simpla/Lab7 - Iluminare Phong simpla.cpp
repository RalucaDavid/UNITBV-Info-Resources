...
{
	if (key == GLFW_KEY_A && action == GLFW_PRESS && g_fKa < 1.1f)
	{
		g_fKa += 0.1;
	}
	if (key == GLFW_KEY_Z && action == GLFW_PRESS && g_fKa > 0.0f)
	{
		g_fKa -= 0.1;
	}
	if (key == GLFW_KEY_D && action == GLFW_PRESS && g_fKd< 1.1f)
	{
		g_fKd += 0.1;
	}
	if (key == GLFW_KEY_C && action == GLFW_PRESS && g_fKd > 0.0f)
	{
		g_fKd -= 0.1;
	}
	...
	if (key == GLFW_KEY_F && action == GLFW_PRESS && g_fe > 1.0f)
	{
		g_fe /= 2;
	}
}
...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		lightPos.x = fRadius * std::sin(currentFrame/2);
		lightPos.y = fRadius * std::cos(currentFrame/2);
		...
		lightingShader.SetValue("n", g_fe);
